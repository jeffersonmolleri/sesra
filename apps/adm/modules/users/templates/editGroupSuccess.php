<?php $group = $form->getObject() ?>
<div class="content">
  <?php if ($sf_user->hasCredential('users')) : ?>
  	<h1><?php echo __('Editando Grupo') ?>: <?php echo $group->getName() ?></h1>
  
  	<?php include '_group_form.php' ?>
  <?php else : ?>
...
<div id="side">
  <div class="block" id="actions">
    <h2><?php echo __('Ações') ?></h2>
      <?php echo link_to(image_tag('ico16/btprev') . __('retornar à lista'), 'users/groups') ?>
  </div>
</div>
