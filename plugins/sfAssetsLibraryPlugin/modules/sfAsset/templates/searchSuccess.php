...
<div class="content">
  ...
  <div id="sf_asset_container"<?php $popup and print ' style="overflow-y: scroll; height: 410px;"'?>>
    <?php foreach ($pager->getResults() as $sf_asset): ?>
      <div class="search_result">
        <?php include_partial('sfAsset/asset', array('sf_asset' => $sf_asset, 'is_search' => true)) ?>
        <div class="details">
          <?php echo assets_library_breadcrumb($sf_asset->getFolderPath(ESC_RAW), true, 'list') ?><?php echo link_to_asset_action($sf_asset->getFilename(), $sf_asset) ?><br />
          <?php if ($description = $sf_asset->getDescription(ESC_RAW)): ?>
            <?php echo $description ?><br />
          ...
          <?php if ($author = $sf_asset->getAuthor(ESC_RAW)): ?>
            <?php echo __('Author: %author%', array('%author%' => $author), 'sfAsset') ?><br />
          <?php endif; ?>
          <?php echo __('Created on %date%', array('%date%' => format_date($sf_asset->getCreatedAt('U'))), 'sfAsset') ?><br />
...
<div id="side">
  <div id="actions" class="block">
    <h2>Ações</h2>
    <?php echo link_to(image_tag('ico16/btprev') . __('Back to the list', null, 'sfAsset'), 'sfAsset/list') ?>
    <?php include_partial('sfAsset/sidebar_search', array('search_params' => $search_params)) ?>
  </div>
  <div id="sf_asset_bar" class="block">
    <h2>Opções</h2>
    <?php include_partial('sfAsset/sidebar_sort') ?>
  </div>
</div>
